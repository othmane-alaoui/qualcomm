...
parser = argparse.ArgumentParser(description='Choose your model, test, number of test and if you want to save the resultats.')
...
def loadModel(name):
    model = YOLOv3Predict(path1+"/"+str(name)) 
    model.initialize()
    return model
...
def predictModel(model, test_type):
    ...
    pred = session.run([], input_feed=feed_dict)
    return pred
...
yolo3pred = inspect.getsource(YOLOv3Predict)
...
save_data.append(to_save)
print('duree de l\'evaluation memoire/cpu : '+str(to_save)+' secondes')
print('')

session = onnxruntime.InferenceSession(str(path1)+'/'+str(model_name[nb_model]))
...
save_data.append(to_save)
print('duree total du test : '+str(to_save)+' secondes')
print('')
#print(save_data)
